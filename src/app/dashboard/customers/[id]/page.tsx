    ...
    <div>
      ...
      <div className="page-header">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <Link
            href={`/dashboard/customers/new?id=${id}`}
      ...
      {status === "Expiring Soon" && (
        <div className="alert alert-warning">
          ...
          <div>
            <strong>Renewal Alert:</strong> This policy expires in {days} day
            {days !== 1 ? "s" : ""} on {formatDate(policy.endDate)}. Please
            contact the customer for timely renewal.
          </div>
        </div>
      )}
      {status === "Expired" && (
        <div className="alert alert-danger">
          <AlertTriangle size={18} style={{ flexShrink: 0 }} />
          <div>
            <strong>Policy Expired:</strong> This policy expired on{" "}
            {formatDate(policy.endDate)}. Customer is currently uninsured.
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div className="detail-grid">
              ...
              <div className="detail-field" style={{ gridColumn: "1 / -1" }}>
                ...
                >
                  <MapPin
                    size={13}
                    style={{ color: "var(--primary)", marginTop: 3 }}
                  />{" "}
                  {policy.address || "—"}
                </div>
              </div>
        ...
        {policy.type === "motor" && (
          <div className="card">
            <div className="card-header">
              <span className="card-title" style={{ color: "var(--motor)" }}>
                🚗 Vehicle Details
              </span>
            </div>
            <div className="card-body">
        ...
        {policy.type === "medical" && (
          <div className="card">
            <div className="card-header">
              <span className="card-title" style={{ color: "var(--medical)" }}>
                🏥 Health Details
              </span>
            </div>
            <div className="card-body">
              ...
              {Array.isArray(policy.members) && policy.members.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  ...
                  >
                    {policy.members.map(
                      (m: { name?: string; age?: string }, i: number) => (
                        <div
                          key={i}
                          style={{
                            display: "flex",
        ...
        {policy.type === "fire" && (
          <div className="card">
            <div className="card-header">
              <span className="card-title" style={{ color: "var(--fire)" }}>
                🏠 Property Details
              </span>
            </div>
            <div className="card-body">
              <div className="detail-grid">
                ...
                <Field
                  ...
                  value={policy.constructionType}
                />
                <Field
                  label="Built-up Area"
                  value={`${policy.builtUpArea} sq ft`}
                />
                <Field
                  label="Property Value"
                  value={formatCurrency(policy.propertyValue)}
                />
                <Field
                  label="Stock Value"
                  value={formatCurrency(policy.stockValue)}
                />
        ...
        {policy.type === "life" && (
          <div className="card">
            <div className="card-header">
              <span className="card-title" style={{ color: "var(--life)" }}>
                💼 Life Policy Details
              </span>
            </div>
            <div className="card-body">
              <div className="detail-grid">
                ...
                <Field
                  label="Annual Income"
                  value={formatCurrency(policy.annualIncome)}
                />
                <Field label="Smoker" value={policy.smoker} />
        ...
        {policy.type === "marine" && (
          <div className="card">
            <div className="card-header">
              <span className="card-title" style={{ color: "var(--marine)" }}>
            ...
            <div className="card-body">
              <div className="detail-grid">
                <Field
                  ...
                  value={policy.marineInsuranceType}
                />
          ...
          <div className="card">
            ...
            <div className="card-body">
              <div className="detail-grid">
                ...
                <Field
                  label="Total Wages"
                  value={formatCurrency(policy.totalWages)}
                />
                <Field label="Risk Category" value={policy.riskCategory} />
                <Field
                  label="Coverage Location"
                  value={policy.coverageLocation}
                />
                <Field
                  label="Employer Liability Limit"
                  value={formatCurrency(policy.employerLiabilityLimit)}
                />
              </div>
            </div>
          </div>
        ...
        {policy.type === "travel" && (
          <div className="card">
            <div className="card-header">
              <span className="card-title" style={{ color: "var(--travel)" }}>
            ...
            <div className="card-body">
              <div className="detail-grid">
                ...
                <Field label="Visa Type" value={policy.visaType} />
                <Field
                  label="Trip Dates"
                  value={`${formatDate(policy.tripStartDate)} to ${formatDate(policy.tripEndDate)}`}
                />
                <Field
                  label="Destinations"
                  value={policy.destination?.join?.(", ")}
                />
                <Field label="Coverage Type" value={policy.coverageType} />
                <Field
                ...
                <Field
                  ...
                  value={policy.activitiesCovered?.join?.(", ")}
                />
                <Field
                  label="Pre-Existing Conditions"
                  value={policy.preExistingConditions || "None"}
              ...
              {Array.isArray(policy.travelers) &&
                policy.travelers.length > 0 && (
                  <div style={{ marginTop: 20 }}>
                    <div
                      className="form-section-title"
                      style={{ fontSize: 12, marginBottom: 10 }}
                    >
                      Travelers ({policy.numberOfTravelers})
                    </div>
                    <div
                      style={{
                        display: "flex",
                    ...
                    >
                      {policy.travelers.map((t: any, i: number) => (
                        <div
                          key={i}
                          style={{
                            display: "flex",
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title">
              <FileText size={15} style={{ color: "var(--primary)" }} /> Policy
              Information
            </span>
          </div>
          <div className="card-body">
        ...
        >
          Added on {formatDate(policy.createdAt)} · Last updated{" "}
      ...
      {deleteConfirm && (
        ...
        >
          <div className="confirm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              This will permanently remove this customer and all policy data.
              This action cannot be undone.
            </p>
            {deleteError && (
              <p style={{ color: "var(--status-expired)", fontSize: 13 }}>
                {deleteError}
              </p>
            )}
